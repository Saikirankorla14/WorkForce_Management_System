...
from main.models import Appointment, AppointmentRequest, AppointmentRescheduleHistory, Attendance, CustomUser, DayOff, Farm, Crop, Feedback, Leave, Payment, Payment_Worker, Resource, Person, FarmingDates, FarmingCosts, FarmProduce, FarmVisitRequest, FarmPhoto, FarmVisitReport, Scheduler, Service, Task, Worker, WorkingHours
from django.core.validators import MaxValueValidator, MinValueValidator
from django.core.exceptions import ValidationError
from django.utils.translation import gettext_lazy as _
from phonenumber_field.formfields import PhoneNumberField
from phonenumber_field.widgets import PhoneNumberPrefixWidget
from main.utils.db_helpers import get_user_model
...
class FarmForm(forms.ModelForm):
    class Meta:
        model = Farm
        fields = ['district', 'other_location', 'location_coordinates', 'land_size', 'crops', 'other_crops','services_required','other_service']
    ...
    other_service = forms.CharField(required=False, widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Other Service', 'style': 'display:none;'}))

    # services_required = forms.ChoiceField(choices=SERVICES_CHOICES, widget=forms.Select(attrs={'class': 'form-control'}))
    ...
    def clean(self):
        ...
        district = cleaned_data.get('district')
        other_location = cleaned_data.get('other_location')
        services_required = cleaned_data.get('services_required',[])
        other_service = cleaned_data.get('other_service')
        # Validate that 'other_location' is provided if 'Other' is selected
        if district == 'Other' and not other_location:
            raise forms.ValidationError("Please provide an 'Other' location.")
        if 'Other' in services_required and not other_service:
            raise forms.ValidationError("Please provide an 'Other' Service.")
...
class FeedbackForm(forms.ModelForm):
    class Meta:
        model = Feedback
        fields = ['service_name', 'rating', 'comments']
