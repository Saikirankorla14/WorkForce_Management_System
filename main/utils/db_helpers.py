...
from main.utils.date_time import combine_date_and_time, get_weekday_num

Appointment = apps.get_model('main', 'Appointment')
AppointmentRequest = apps.get_model('main', 'AppointmentRequest')
WorkingHours = apps.get_model('main', 'WorkingHours')
DayOff = apps.get_model('main', 'DayOff')
PaymentInfo = apps.get_model('main', 'PaymentInfo')
Worker = apps.get_model('main', 'Worker')
Config = apps.get_model('main', 'Config')
Service = apps.get_model('main', 'Service')
EmailVerificationCode = apps.get_model('main', 'EmailVerificationCode')
AppointmentRescheduleHistory = apps.get_model('main', 'AppointmentRescheduleHistory')
...
def schedule_email_reminder(appointment, request, appointment_datetime=None):
    ...
    from main.settings import check_q_cluster
    ...
    relative_reschedule_url = reverse('prepare_reschedule_appointment', args=[ar_id_request])
...
def get_staff_member_appointment_list(staff_member: Worker) -> list: # type: ignore
...
def get_staff_member_buffer_time(staff_member: Worker, date: datetime.date) -> float: # type: ignore
...
def get_staff_member_end_time(staff_member: Worker, date: datetime.date) -> Optional[datetime.time]: # type: ignore
...
def get_staff_member_slot_duration(staff_member: Worker, date: datetime.date) -> int: # type: ignore
...
def get_staff_member_start_time(staff_member: Worker, date: datetime.date) -> Optional[datetime.time]: # type: ignore
...
def is_working_day(staff_member: Worker, day: int) -> bool: # type: ignore
