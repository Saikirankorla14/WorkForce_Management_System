...
from main.forms import PersonalInformationForm, ServiceForm, StaffDaysOffForm, StaffWorkingHoursForm
from main.messages import appt_updated_successfully
from main.settings import APPOINTMENT_PAYMENT_URL
from main.utils.date_time import (
...
from main.utils.json_context import convert_appointment_to_json, get_generic_context, json_response
from main.utils.permissions import check_entity_ownership
from main.utils.session import handle_email_change
...
def prepare_user_profile_data(user, staff_user_id):
    ...
    if user.is_superuser and staff_user_id is None:
        ...
        btn_staff_me_link = reverse('make_superuser_staff_member')
        if Worker.objects.filter(user=user).exists():
            btn_staff_me = "Remove me"
            btn_staff_me_link = reverse('remove_superuser_staff_member')
        data = {
            'error': False,
    ...
    if not staff_member:
        return {
            'error': True,
            'extra_context': {'message': _("Not authorized."), 'back_url': reverse('user_profile')},
            'status_code': 403
        }
